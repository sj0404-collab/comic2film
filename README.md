...
- **🆓 без ключа**: Pollinations — провайдер, работающий совсем без ключа (`text.pollinations.ai/openai`). **Протестировано вживую: отвечают все 5 анонимных моделей** — `openai`, `openai-fast`, `gpt-oss`, `gpt-oss-20b`, `ovh-reasoning` (изредка при параллельных вызовах с одного IP Pollinations отдаёт 429 «queue full» — это rate limit, не поломка; список обновляется кнопкой «🔄 Модели без ключа»). Плюс **демо-модели OpenCode Zen** — big-pickle и др. бесплатны по тарифу и отдаются без ключа, но гейт free tier пропускает только запросы «как opencode CLI» (identity-заголовки: `User-Agent: opencode/1.18.31`, `x-opencode-session/request/client/project`, `stream:true` + официальные тулы). Браузер не может отправить такие заголовки сам (нельзя переопределить `User-Agent`, у zen нет CORS), поэтому без ключа они работают **через мини-релей** `relay/zen-relay.mjs` (URL — в настройках): `node relay/zen-relay.mjs` → в настройках «URL Zen-релея: http://localhost:8789`. **Протестировано вживую через релей**: big-pickle, mimo-v2.5-free, mimo-v2.6-flash-free, nemotron-3-ultra-free, nemotron-3.5-lightning-free, ling-3.0-flash-fin-free, space-bunny-free — отвечают. Имена «free» (qwen3.6-plus-free и т.п.) из моделей-каталога free tier анонимно не отдаются (401/500) — в curated-список не включены.
