...
- **🆓 без ключа**: Pollinations — единственный провайдер, работающий совсем без ключа (`text.pollinations.ai/openai`). Список живых бесплатных моделей (tier=anonymous) обновляется кнопкой «🔄 Модели без ключа». Пикер по умолчанию открывается с фильтром «🆓 без ключа» — сразу видны модели без ключа (обратите внимание: «бесплатные» модели OpenCode Zen (big-pickle и др.) бесплатны по тарифу, но требуют регистрацию ключа Zen).
